...
async def test_controller_create_should_return_success(client,products_url):
    ...
    assert content == {"name":"Iphone 14 Pro","quantity":10,"price":"8.500","status":True}
...
async def test_controller_get_should_return_success(client,products_url,product_inserted):
    ...
    assert content == {"id": str(product_inserted.id),"name":"Iphone 14 Pro","quantity":10,"price":"8.500","status":True}

async def test_controller_get_should_return_not_found(client,products_url,product_inserted):
...
async def test_controller_patch_should_return_success(client,products_url,product_inserted):
    ...
    assert response.status_code==status.HTTP_200_OK
    assert content == {"id": str(product_inserted.id),"name":"Iphone 14 Pro","quantity":10,"price":"7.500","status":True}
